...
func bruteForce(obj string, winner chan<- solution, possibilities <-chan try, done <-chan struct{}) {
	...
	if uint32(len(obj)) > config.Config.LeafSize {
	...
	leafHash := blake2.New(&blake2.Config{Size: 64, Tree: &blake2.Tree{Fanout: 0, MaxDepth: 2, LeafSize: config.Config.LeafSize, NodeOffset: 0, NodeDepth: 0, InnerHashSize: 64, IsLastNode: true}})
	rootHash := blake2.New(&blake2.Config{Size: 64, Tree: &blake2.Tree{Fanout: 0, MaxDepth: 2, LeafSize: config.Config.LeafSize, NodeOffset: 0, NodeDepth: 1, InnerHashSize: 64, IsLastNode: true}})
